...
impl IndexingOptimizer {
    ...
    fn worst_segment(
        ...
        //
        // For incremental HNSW: always pair the unindexed segment with the
        // largest indexed segment, even if the combined size exceeds
        // max_segment_size. This gives try_incremental_build both segments
        // together so it can insert only the new vectors into the existing
        // graph instead of building from scratch twice (once for the small
        // segment alone, then again when the merge optimizer combines them).
        let largest_indexed = segments_read_guard
            .iter()
            .filter(|(idx, _)| !excluded_ids.contains(idx))
            .filter_map(|(idx, segment)| {
                let segment_entry = segment.get();
        ...
        if let Some((idx, _size)) = largest_indexed {
            if idx != selected_segment_id {
                // Always pair with the largest indexed segment so
                // try_incremental_build can avoid a full rebuild.
                return vec![selected_segment_id, idx];
            }
        }

        // Fall back: optimize only the unindexed segment
