...
// Mirror the constant from hnsw.rs so from_graph can use it.
...
impl GraphLayersBase for GraphLayersBuilder {
        ...
        F: FnMut(PointOffsetType),
    {
        ...
        for link in links.iter() {
            ...
            if idx < ready_list.len() && ready_list[idx] {
                f(*link);
            }
        }
    }
...
impl GraphLayersBuilder {
    ...
    /// Reconstruct a `GraphLayersBuilder` from an already-built `GraphLayers`.
    ///
    /// All existing points and their links are imported so that new points can
    /// be inserted into the live graph via [`link_new_point`] without requiring
    /// a full rebuild.  Imported points are immediately marked as ready so they
    /// participate in neighbour-search for the newly inserted points.
    pub fn from_graph<TGraphLinks: GraphLinks>(
        graph: &GraphLayers<TGraphLinks>,
        entry_points_num: usize,
    ) -> Self {
        let num_points = graph.links.num_points();

        let mut builder = Self::new_with_params(
            num_points,
            graph.m,
            graph.m0,
            graph.ef_construct,
        ...
        // Import each point's level structure and links.
        for point_id in 0..num_points as PointOffsetType {
            let level = graph.links.point_level(point_id);
            builder.set_levels(point_id, level);

            for lvl in 0..=level {
                let existing = graph.links.links(point_id, lvl);
                if !existing.is_empty() {
                    let mut locked = builder.links_layers[point_id as usize][lvl].write();
                    locked.extend_from_slice(existing);
                }
            }
        }

        // Import entry points.
        let imported_entry_points = graph.entry_points.clone();
        *builder.entry_points.lock() = imported_entry_points;

        // Mark all existing points as ready so they are visible during
        // neighbour-search when new points are linked.
        {
            let mut ready = builder.ready_list.write();
            for i in 0..num_points {
                ready.set(i, true);
            }
        }

        // Update max_level to match the existing graph.
        let max_lvl = (0..num_points as PointOffsetType)
            .map(|pid| graph.links.point_level(pid))
            .max()
            .unwrap_or(0);
        builder
            .max_level
            .store(max_lvl, std::sync::atomic::Ordering::Relaxed);
    ...
    pub fn set_levels(&mut self, point_id: PointOffsetType, level: usize) {
        ...
        // Grow ready_list in sync with links_layers so link_new_point can
        // safely mark this point as ready without an out-of-bounds panic.
        {
            let mut ready = self.ready_list.write();
            if ready.len() <= point_id as usize {
                ready.resize(point_id as usize + 1, false);
            }
        }
...
mod tests {
    ...
    /// `from_graph` must faithfully reproduce every level-0 link from the
    /// original frozen `GraphLayers` so that incremental insertions can build
    /// on an already-established neighbourhood structure.
    #[test]
    fn test_from_graph_preserves_all_links() {
        use crate::index::hnsw_index::tests::create_graph_layer_fixture;

        let num_vectors = 200;
        let dim = 8;
        let m = M; // 8, defined at top of tests module

        let mut rng = StdRng::seed_from_u64(99);

        let (_holder, original) =
            create_graph_layer_fixture::<CosineMetric, _>(num_vectors, m, dim, true, &mut rng, None);

        // Round-trip: frozen GraphLayers → mutable GraphLayersBuilder
        let builder = GraphLayersBuilder::from_graph(&original, 10);

        // --- Level-0 links must match exactly ---
        for pid in 0..num_vectors as PointOffsetType {
            let orig: Vec<_> = original.links.links(pid, 0).to_vec();
            let built: Vec<_> = builder.links_layers[pid as usize][0].read().clone();
            assert_eq!(
                orig, built,
                "level-0 links for point {pid} must be identical after from_graph"
            );
        }

        // --- All imported points must be marked ready ---
        {
            let ready = builder.ready_list.read();
            for pid in 0..num_vectors {
                assert!(
                    ready[pid],
                    "point {pid} must be marked ready after from_graph"
                );
            }
        }

        // --- Entry points must be preserved ---
        let ep = builder
            .entry_points
            .lock()
            .get_entry_point(|_| true)
            .expect("entry point must exist after from_graph");
        let orig_ep = original
            .entry_points
            .get_entry_point(|_| true)
            .expect("entry point must exist in original graph");
        assert_eq!(
            ep.point_id, orig_ep.point_id,
            "entry point must be preserved by from_graph"
        );

        // --- Multi-level points: all levels must be preserved ---
        for pid in 0..num_vectors as PointOffsetType {
            let orig_level = original.links.point_level(pid);
            let built_levels = builder.links_layers[pid as usize].len() - 1;
            assert_eq!(
                orig_level, built_levels,
                "level count for point {pid} must match after from_graph"
            );
            for lvl in 1..=orig_level {
                let orig: Vec<_> = original.links.links(pid, lvl).to_vec();
                let built: Vec<_> = builder.links_layers[pid as usize][lvl].read().clone();
                assert_eq!(
                    orig, built,
                    "level-{lvl} links for point {pid} must be identical after from_graph"
                );
            }
        }
    }

    /// A newly inserted point (via `link_new_point` on a builder produced by
    /// `from_graph`) must be reachable from a search on the resulting graph.
    #[test]
    fn test_incremental_insert_via_from_graph_is_searchable() {
        use crate::fixtures::index_fixtures::{FakeFilterContext, TestRawScorerProducer};
        use crate::index::hnsw_index::tests::create_graph_layer_fixture;

        let num_vectors = 150usize;
        let dim = 8;
        let m = M;
        let ef_construct = 16;
        ...
        // Build a graph on `num_vectors` points.
        let (_holder, original) =
            create_graph_layer_fixture::<CosineMetric, _>(num_vectors, m, dim, true, &mut rng, None);

        // Create an extended storage that includes a (num_vectors+1)-th vector.
        let new_id = num_vectors as PointOffsetType;
        let mut extended_holder =
            TestRawScorerProducer::<CosineMetric>::new(dim, num_vectors + 1, &mut rng);
        // The last slot was filled by TestRawScorerProducer::new; grab it as our query.
        let new_vec = extended_holder.vectors.get(new_id).to_vec();

        // Lift frozen graph into a mutable builder and insert the new point.
        let mut builder = GraphLayersBuilder::from_graph(&original, 10);
        let level = builder.get_random_layer(&mut rng);
        builder.set_levels(new_id, level);

        let fake_filter = FakeFilterContext {};
        let raw_scorer = extended_holder.get_raw_scorer(new_vec.clone()).unwrap();
        let scorer = FilteredScorer::new(raw_scorer.as_ref(), Some(&fake_filter));
        builder.link_new_point(new_id, scorer);

        let graph = builder
            .into_graph_layers::<GraphLinksRam>(None)
            .unwrap();

        // Querying with new_vec should return new_id as the top result.
        let raw_scorer2 = extended_holder.get_raw_scorer(new_vec).unwrap();
        let scorer2 = FilteredScorer::new(raw_scorer2.as_ref(), Some(&fake_filter));
        let results = graph.search(1, ef_construct, scorer2, None);

        assert_eq!(
            results.first().map(|s| s.idx),
            Some(new_id),
            "newly inserted point must be top-1 when queried with its own vector"
        );
    }
}
