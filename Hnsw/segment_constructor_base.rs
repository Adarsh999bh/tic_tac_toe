...
/// Copy an existing segment directory to a new location and load it.
///
/// This is the foundation of true incremental HNSW indexing: instead of
/// rebuilding an already-indexed segment from scratch, we clone its on-disk
/// state and then only insert new vectors via `flush_pending`.
///
/// # Arguments
///
/// * `src`     - Path of the segment directory to copy (must already exist and be complete).
/// * `dst_dir` - Parent directory where a new UUID-named folder will be created.
/// * `stopped` - Cancellation flag.
///
/// # Returns
///
/// The freshly-loaded copy of the segment, ready for incremental updates.
pub fn copy_segment(
    src: &Path,
    dst_dir: &Path,
    stopped: &AtomicBool,
) -> OperationResult<Segment> {
    let dst = dst_dir.join(Uuid::new_v4().to_string());
    copy_dir_all(src, &dst)?;
    let segment = load_segment(&dst, stopped)?.ok_or_else(|| {
        OperationError::service_error(format!(
            "Copied segment could not be loaded from {}",
            dst.display()
        ))
    })?;
    Ok(segment)
...
fn copy_dir_all(src: &Path, dst: &Path) -> OperationResult<()> {
