...
/// File name for persisting pending (not-yet-indexed) vector IDs across restarts.
const PENDING_IDS_FILE: &str = "pending_ids.bin";
...
pub struct HNSWIndex<TGraphLinks: GraphLinks> {
    ...
    /// Points added via `update_vector` that are not yet folded into the HNSW
    /// graph. They are searched with a plain linear scan and merged with graph
    /// results at query time, giving immediate read-your-writes visibility.
    /// When the list grows beyond `config.full_scan_threshold` a warning is
    /// emitted recommending a full `build_index` call.
    pending_ids: Vec<PointOffsetType>,
}
...
impl<TGraphLinks: GraphLinks> HNSWIndex<TGraphLinks> {
    pub fn open(
        ...
        // Restore any pending IDs that were saved before the last restart.
        // This ensures vectors registered via update_vector but not yet
        // flushed into the graph remain visible after a crash or restart.
        let pending_ids_path = path.join(PENDING_IDS_FILE);
        let pending_ids: Vec<PointOffsetType> = if pending_ids_path.exists() {
            read_bin(&pending_ids_path).unwrap_or_default()
        } else {
            vec![]
        };
        Ok(HNSWIndex {
    ...
    /// Number of vectors added via `update_vector` that have not yet been
    /// folded into the HNSW graph by a call to `build_index`.
    pub fn pending_vector_count(&self) -> usize {
        self.pending_ids.len()
    }

    /// Incrementally insert all pending vectors into the HNSW graph without
    /// performing a full rebuild.
    ///
    /// **How it works**
    /// 1. The existing frozen `GraphLayers` is converted back into a mutable
    ///    `GraphLayersBuilder` (all current links are preserved).
    /// 2. Each pending vector is given a random level and linked into the
    ///    graph exactly as during a normal build.
    /// 3. The builder is frozen back into a new `GraphLayers` and saved.
    ///
    /// **When to prefer this over `build_index`**
    /// - A small number of new vectors (< `full_scan_threshold`) have been
    ///   added and you want them to benefit from graph-quality search without
    ///   paying for a full O(n log n) rebuild.
    /// - You want near-zero downtime: the index remains searchable (via the
    ///   pending plain-scan path) while this runs, and after it returns all
    ///   pending points are fully graph-indexed.
    ///
    /// For large batches a full `build_index` will produce better graph
    /// quality because it can lay out levels globally.
    pub fn flush_pending(
        ...
        let total_new = self.pending_ids.len();
        let current_indexed = self
            .graph
            .as_ref()
            .map(|g| g.num_points())
            .unwrap_or(0);
        let total_after = current_indexed + total_new;
        let entry_points_num = (total_after
            .checked_div(self.config.full_scan_threshold)
            .unwrap_or(0)
            * 10)
            .max(1);

        // ----- Step 1: lift existing graph into a mutable builder -----
        let mut builder = match &self.graph {
            Some(graph) => GraphLayersBuilder::from_graph(graph, entry_points_num),
            None => GraphLayersBuilder::new(
                total_after,
                self.config.m,
                self.config.m0,
        ...
        // ----- Step 2: assign levels and link pending points -----
        for &point_id in &self.pending_ids {
            check_process_stopped(stopped)?;
            // Skip vectors that were deleted before we got here.
            if deleted_bitslice
        ...
        for &point_id in &self.pending_ids {
            ...
            let vector = vector_storage.get_vector(point_id);
            ...
            builder.link_new_point(point_id, points_scorer);
        }

        // ----- Step 3: freeze back into a GraphLayers and persist -----
        let graph_links_path = GraphLayers::<TGraphLinks>::get_links_path(&self.path);
        self.graph = Some(builder.into_graph_layers(Some(&graph_links_path))?);

        let new_indexed = self
            .config
            .indexed_vector_count
            .get_or_insert(0);
        *new_indexed = current_indexed + total_new;
    ...
    fn save_pending_ids(&self) -> OperationResult<()> {
        let pending_path = self.path.join(PENDING_IDS_FILE);
        if self.pending_ids.is_empty() {
            ...
            atomic_save_bin(&pending_path, &self.pending_ids)?;
        }
        Ok(())
    }
    ...
    fn search_with_graph(
        ...
        let mut graph_results = match &self.graph {
            Some(graph) => {
                let search_result =
        ...
        // -------------------------------------------------------------------
        // Incremental: merge in any pending (not-yet-indexed) points so that
        // newly upserted vectors are immediately visible in search results.
        // -------------------------------------------------------------------
        if !self.pending_ids.is_empty() {
            let pending_scorer = Self::construct_search_scorer(
                vector,
                &vector_storage,
                quantized_vectors.as_ref(),
                id_tracker.deref(),
            ...
            // Score every pending point that passes the optional filter.
            let filter_context2 = filter.map(|f| payload_index.filter_context(f));
            let mut eligible: Vec<PointOffsetType> = self
                .pending_ids
                .iter()
                .copied()
                .filter(|&pid| {
                    ...
                    !vector_storage
                        ...
                        && filter_context2
                            .as_ref()
                            .map(|ctx| ctx.check(pid))
                            .unwrap_or(true)
                })
                .collect();

            let pending_results =
                pending_scorer.peek_top_iter(&mut eligible.iter().copied(), oversampled_top);

            // Merge and keep top-N by score (scores are similarities; higher = better).
            graph_results.extend(pending_results);
            graph_results.sort_unstable_by(|a, b| b.score.partial_cmp(&a.score).unwrap());
            graph_results.dedup_by_key(|r| r.idx);
            graph_results.truncate(top);
        }
    ...
    fn search_plain(
        ...
        // filtered_points comes from the payload index and already excludes
        // deleted vectors, but newly pending points are not yet in the payload
        // index.  Build a combined iterator that covers both.
        let pending_not_already_included: Vec<PointOffsetType> = self
            .pending_ids
            .iter()
            .copied()
            .filter(|pid| {
                !filtered_points.contains(pid)
                    && !vector_storage
                        .deleted_vector_bitslice()
                        .get(*pid as usize)
                        .map(|b| *b)
        ...
        let mut combined: Vec<PointOffsetType> =
            filtered_points.iter().copied().chain(pending_not_already_included).collect();

        let search_result = raw_scorer.peek_top_iter(&mut combined.iter().copied(), oversampled_top);
...
impl<TGraphLinks: GraphLinks> VectorIndex for HNSWIndex<TGraphLinks> {
    ...
    fn build_index(&mut self, permit: Arc<CpuPermit>, stopped: &AtomicBool) -> OperationResult<()> {
        ...
        // All vectors are now in the graph; the pending list is no longer needed.
        self.pending_ids.clear();
    ...
    fn files(&self) -> Vec<PathBuf> {
        let mut files = if self.graph.is_some() {
            vec![
        ...
        // Include the pending IDs file if it exists so that snapshots and
        // shard transfers capture un-flushed pending registrations.
        let pending_path = self.path.join(PENDING_IDS_FILE);
        if pending_path.exists() {
            files.push(pending_path);
        }
        files
    }

    fn indexed_vector_count(&self) -> usize {
        // Pending vectors are not yet in the graph but are searchable, so we
        // include them in the total to give an accurate picture of coverage.
        let graph_count = self
            .config
            .indexed_vector_count
            .or_else(|| self.graph.as_ref().map(|g| g.num_points()))
            .unwrap_or(0);
        graph_count + self.pending_ids.len()
    }

    fn update_vector(&mut self, id: PointOffsetType, _vector: VectorRef) -> OperationResult<()> {
        // The vector data has already been written to `vector_storage` by the
        // caller before this method is invoked.  We just need to register the
        // point so that:
        //   1. It is included in the next `build_index` (full rebuild), AND
        //   2. It is immediately searchable via the pending plain-scan path in
        //      `search_with_graph`.

        // Avoid duplicate entries if the same ID is updated more than once
        // between rebuilds (e.g. an upsert).
        if !self.pending_ids.contains(&id) {
            self.pending_ids.push(id);
            // Persist immediately so this registration survives a crash or restart.
            self.save_pending_ids()?;
        }

        // Warn the user when the pending list is large enough that search
        // quality may degrade, and a full `build_index` is advisable.
        let threshold = self.config.full_scan_threshold;
        if self.pending_ids.len() >= threshold {
            log::warn!(
                "HNSW index has {} pending (un-indexed) vectors (threshold={}). \
                 Search quality may degrade. Call build_index to consolidate.",
                self.pending_ids.len(),
                threshold,
            );
        }
