...
pub trait SegmentOptimizer {
    ...
    ///
    /// This path is taken when exactly one of the `optimizing_segments` is an
    /// already-indexed HNSW segment and the rest are small appendable (Plain)
    /// segments whose total vector count is less than 25% of the existing
    /// indexed segment. Instead of rebuilding the entire graph from scratch we:
    ///
    /// 1. Copy the indexed segment directory to a new location.
    /// 2. Load it (preserving the existing HNSW graph).
    /// 3. Upsert only the new vectors from the appendable segments.
    /// 4. Call `flush_pending` on each vector index to incrementally link
    ///    the new vectors into the live graph.
    ///
    /// Returns `None` when the conditions are not met, falling back to the
    /// full rebuild path in `build_new_segment`.
    fn try_incremental_build(
                ...
                LockedSegment::Original(arc) => {
                    ...
                    if guard.segment_type() == SegmentType::Indexed {
                        if indexed_segment.is_some() {
                            // More than one indexed segment — must do full rebuild
                            return Ok(None);
                        }
        ...
        let indexed_seg = match indexed_segment {
            Some(s) => s,
            None => return Ok(None), // Nothing indexed yet — full build needed
        };
        ...
        // ── 2. Count vectors to decide if incremental is worthwhile ───────
        ...
        // Only use incremental path when new vectors are < 25% of existing.
        // Larger batches benefit more from a full rebuild's globally-optimal
        // level assignment.
        if new_count == 0 || new_count >= existing_count / 4 {
            log::debug!(
                "Incremental HNSW skipped: existing={existing_count}, new={new_count} (>= 25% threshold)"
            );
            return Ok(None);
        }

        log::info!(
            "Incremental HNSW: copying indexed segment ({existing_count} vectors), \
             then inserting {new_count} new vectors without full rebuild"
        );

        // ── 3. Copy the indexed segment to a fresh directory ──────────────
        ...
        // ── 4. Upsert new vectors into the copied segment ─────────────────
        for app_seg in &appendable_segments {
            ...
            for point_id in app_guard.iter_points() {
                check_process_stopped(stopped)?;

                // Skip points deleted while optimization was running
                if proxy_deleted_points.read().contains(&point_id) {
                    continue;
                }

                let vectors = app_guard.all_vectors(point_id)?;
                let payload = app_guard.payload(point_id)?;
                let op_num = segment.version();

                segment.upsert_point(op_num, point_id, vectors)?;
                if !payload.is_empty() {
                    segment.set_full_payload(op_num, point_id, &payload)?;
                }
            }
        }

        // ── 5. Flush pending vectors into the HNSW graph ──────────────────
        for (vector_name, vector_data) in &segment.vector_data {
            ...
            if pending > 0 {
                log::info!(
                    "flush_pending: inserting {pending} vectors into existing HNSW graph \
        ...
        // ── 7. Apply point deletions that arrived during optimization ──────
        for &point_id in proxy_deleted_points.read().iter() {
            let _ = segment.delete_point(segment.version(), point_id);
        }

        log::info!(
            "Incremental HNSW complete: segment now has {} indexed vectors",
